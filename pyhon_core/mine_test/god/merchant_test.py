...
def build_manager_token_to_env(trust_refresh=False):
    ...
    response_str = ''
    try:
        response = requests.post(url, headers=_content_type_from_data, data=manager_data)
...
def get_merchant_info_by_merchant_code(merchant_code: str) -> str:
    conn = config[thread.env]
    url = (conn[
...
def login_user(uid):
    ...
    header = {
        'x-forwarded-for': '103.141.118.111'
    }
    response_str = ''
    try:
        response = requests.post(url, headers=header)
...
def build_user_login_url(user_info_str):
    ...
    response_str = ''
    try:
        response = requests.post(url, headers=header, data=manager_data)
        response_str = response.text
        result_dict = json.loads(response_str)
        if result_dict['code'] != 200:
...
if __name__ == '__main__':
    thread.env = 'prod'
    thread.param = '519281031768620617'
    # thread.param = '512455487938200130'
    # thread.param = 'b9011170093f7235e8795746369b286b5332b63a'
    run_core()
